...
一文: **主張しない。構造を見せる。** 売り物は「整理が信用できる」こと。行き止まり禁止（全詳細ページに名前つきの次）。

| ルール | 内容 |
| :--- | :--- |
| 墨と紙 | 俯瞰（全国・OG・地域ヘッダー）=墨 `#241c13`＋座標グリッド ／ 読む場所=紙 `#f7f4ef` |
| 色の文脈 | 複数ジャンル混在=ジャンル色＋形 ／ **1ジャンルに絞ったときだけ**内部色体系（ラーメン=系統色）。色だけに依存しない |
| 形 | **料理=丸**（土地で生まれた・発祥）／ **食材=四角**（土地が育てる・産地）。同じ棚に平積みしない |
| ジャンルUI | 単一選択タブ禁止。**複数選択フィルタ・既定=すべて表示** |
| 二言語 | 三点セット常置。英訳は説明訳なので見出しに使わない。縦帯: ja=漢字縦書き／en=横倒しラテン。ディフォルメ地図セルは漢字を現地表記として両言語維持 |
| 禁止 | ランキング・「優勢」・煽り・県の単一色塗り・偽和風（筆文字/和柄/鳥居色） |

正典: `.doc/00_concept/05_brand.md`（トーン）／ `.doc/30_features/02_ui_ux.md`（画面）。ビジュアルの一次資料はデザインキャンバス（Artifact「Itadaki Atlas デザイン方向」のコンセプトページ）。
