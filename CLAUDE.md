...
2026-09: 全国区の料理（焼き鳥・餃子・おでん・うなぎ蒲焼・すき焼き・唐揚げ・カレーライス・握り寿司・天ぷら等）は発祥地なしの受け皿アイテムとして持ち、複数の「本場」（第3種別、理由文つき）で土地に結びつける（トップ「ひとつの食べもの、いくつもの本場」は本場2箇所以上を優先して日替わり選定）。「ご当地」というラベルは使わず、「生まれた土地」と「いまの広がり」を分けて言う。詳細ページには部位図（牛・豚・鶏のSVG線画、`src/features/map/CutDiagram.tsx`）と系統図（関係データから描く `LineageTree.tsx`）を本文冒頭に置く。計測は GA4（同意バナー、許可時のみ）+ Cloudflare Web Analytics（`src/components/Analytics.tsx`。IDは `.env.production.local`）。正典は `ia-atlas-content` Skill §2.4・§3、スキーマは `.doc/20_data/01_models.md` §3.6/§3.9。
