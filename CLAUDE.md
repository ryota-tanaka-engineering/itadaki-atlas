...
**確定デザイン・確定情報構造で全ページ型実装済み・本番稼働中（workers.dev、独自ドメイン未設定）。** データ投入だけでサイトが伸びる。投入済み: 棚28・ジャンル14（ラーメン58/焼き鳥24/寿司7/和牛43/うどん13/そば13/米の品種37/発酵調味料20/地鶏35/銘柄豚23/牛肉の部位30/豚肉の部位20/魚56/貝・甲殻類・イカタコ35）+ その他63件 = **477件**・47都道府県・タグ26語彙/付与322件・チェーン11（ラーメン）。2026-09 食材拡張（米・調味料・銘柄牛豚鶏・部位・魚介 327件）は概要（Tier1）のみ投入済みで、本文3章は未投入（次の量産対象）。

2026-09: 本文（3章Markdown ja/en）は**全150件**+出典つきで投入済み（`data/bodies/`・`data/bodies2/` → `scripts/import-bodies.ts`）。関係は143本で全詳細に名前つきの次がある（行き止まりゼロ）。本場（土地との結びつき第3種別）とチェーン橋渡しを実装（正典は `ia-atlas-content` Skill §2.4・§3、スキーマは `.doc/20_data/01_models.md` §3.6/§3.9）。本場は地図にも○ピンで出る（ディフォルメ地図の件数にも含む。全県選択可）。
