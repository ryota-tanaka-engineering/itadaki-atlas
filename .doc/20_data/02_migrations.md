...
| 拡張 | 状態 | 解禁条件 |
| :--- | :--- | :--- |
| **PostGIS** | **使わない（封印）** | 「現在地から近いものを探す」「距離順に並べる」が要件化した時点。詳細と理由は `.doc/10_system/01_architecture.md` §2.1 |

座標は `double precision` の素の列で持つ。解禁時に生成列とインデックスを足すだけで移行できる状態を保つため。

全文検索は Postgres 組み込みの FTS を使う（拡張不要）。
