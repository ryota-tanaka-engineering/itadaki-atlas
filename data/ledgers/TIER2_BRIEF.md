...
出力は scratchpad のみ。リポジトリ /Users/tanakaryouta/workspace/ia-miracolo/itadaki-atlas は読み取り専用。再委譲禁止。一意なスクリプト名（build_tier2_<pref>.py）。5件ごとに途中保存。
...
- 断定しない（「〜とされる」「〜と伝わる」）。感覚描写（味・香り・食感）から入る。「なぜこの形になったのか」は気候・地形・産業・保存の必要・行事など構造的理由
