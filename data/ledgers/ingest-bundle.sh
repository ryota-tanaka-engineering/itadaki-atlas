...
for n in sys.argv[1:]:
    ...
    for it in d.get('items',[]):
        for f in ('summary_ja','body_ja'):
            for m in BAD.finditer(it.get(f) or ''): print('  NG', it['slug'], f, it[f][max(0,m.start()-25):m.end()+25])
        for r in it.get('regions') or []:
            for m in BAD.finditer(r.get('note_ja') or ''): print('  NG', it['slug'], 'region', r.get('city'), r['note_ja'][max(0,m.start()-25):m.end()+25])
PY
